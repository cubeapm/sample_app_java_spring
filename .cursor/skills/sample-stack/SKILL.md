...
If the compose user-bridge has no egress (nested NAT), `ensure` writes `tmp/cloud-net-mode=host` and `stack.sh start` generates `tmp/docker-compose.host.yml` (`network_mode: host`, no `networks`/`ports`). Do not merge `docker-compose.cloud.yml` (`!reset` is ignored by some Compose v2 builds).
